<template>
  <form>
    ...
    <button type="button" class="btn btn-primary" @click="updateCert()">Modifier</button>
  </form>
</template>
...
export default{
  ...
  methods: {
    async getCertById() {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/certificat_medical/${this.$route.params.id}`);
    ...
    async updateCert(){
      await axios.put(`http://127.0.0.1:8000/api/certificat_medical/${this.$route.params.id}`,{
