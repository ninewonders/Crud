...
export default{
  ...
  methods: {
    async getFactureById() {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/facture/${this.$route.params.id}`);
    ...
    async updateFacture(){
      await axios.put(`http://127.0.0.1:8000/api/facture/${this.$route.params.id}`,{
        PRIX:this.prix
      });
