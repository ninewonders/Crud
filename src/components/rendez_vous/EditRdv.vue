...
export default{
  ...
  methods: {
    async getRdvById() {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/rendez_vous/${this.$route.params.id}`);
    ...
    async updatePatient(){
      await axios.put(`http://127.0.0.1:8000/api/rendez_vous/${this.$route.params.id}`,{
        date_rdv:this.date,
        heure_rdv:this.hour,
        nom:this.surname,
        prenom:this.name,
